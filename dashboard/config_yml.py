    ...
    try:
        ...
        for cc in configuration['cards']:
            ...
            if 'image' in cc:
                ...
                cc["image"] = cc['image'] \
                    if cc['image'][0] not in ('/', '.') and '//' not in cc[
                    'image'] and '\\\\' not in cc['image'] and '..' not in cc['image'] \
                    else ''
            # Status
            if 'status_enable' in cc:
                cc["status_enable"] = cc["status_enable"] \
                    if cc["status_enable"] in (True, False) \
                    else True
                if 'status_url' in cc:
                    cc['status_url'] = cc['status_url'] \
                        if (cc['status_url'][0] not in ('/', '.') and '//' not in cc['status_url']
                            and '\\\\' not in cc['status_url'] and '..' not in cc['status_url']) \
                        else ''
                if 'status_code' in cc:
                    cc["status_code"] = cc["status_code"][:Card_code_max_length - 1]
                if 'status_freq' in cc:
                    cc["status_freq"] = cc["status_freq"] \
